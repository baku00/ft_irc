...
void	Mode::execute(Client &client, std::vector<std::string> args) const {
	...
	switch (mode[1])
	{
		...
		case 'k':
			...
			break;
		case 'o':
			std::string operator = this->getValue(args);
			Client *client_operator = ServerInstance::getInstance()->getClientByNickname(operator);

			if (!client_operator)
				return client.reply(ERR_NOSUCHNICK, operator.c_str());
